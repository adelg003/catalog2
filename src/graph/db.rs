...
pub async fn edge_select(
    ...
    query_as!(
        ...
        WITH RECURSIVE
                ...
                UNION ALL
                    SELECT
                     ...
                     INNER JOIN
                        parent p
                     ON
                        d.source_type = p.dest_type
                        AND d.source_id = p.dest_id
                     WHERE
                        p.dest_distance + 1 <= $3 -- parent distance param
            ),
            ...
            child AS (
                ...
                UNION ALL
                    ...
                    INNER JOIN
                        child c
                    ON
                        d.dest_type = c.source_type
                        AND d.dest_id = c.source_id
                    WHERE
                        ABS(c.source_distance - 1) <= $4 -- child distance param
            ),
            ...
            -- Lets expose some use full details to descripbe the node
            details AS (
                    SELECT
                        'model' AS type,
                        id,
        ...
        -- Now we can finaly select what we want
        SELECT
